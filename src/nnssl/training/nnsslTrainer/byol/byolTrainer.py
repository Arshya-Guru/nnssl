...
class BYOLArchitecture(nn.Module):
    def __init__(
        self,
        encoder: nn.Module,
        encoder_output_channels,  # Can be int or list
        ...
        # Handle list of channels (one per encoder stage) - use bottleneck
        if isinstance(encoder_output_channels, (list, tuple)):
            encoder_output_channels = encoder_output_channels[-1]
