...
def get_network_by_name(
    ...
    if encoder_only:
        if architecture_name in ["ResEncL", "NoSkipResEncL"]:
            ...
            try:
                # Store attributes before extracting encoder
                key_to_encoder = getattr(model, 'key_to_encoder', 'stages')
                keys_to_in_proj = getattr(model, 'keys_to_in_proj', [])
                
                model = model.encoder
                
                # Set attributes on encoder with adjusted paths
                model.key_to_encoder = key_to_encoder.replace("encoder.", "") if isinstance(key_to_encoder, str) else "stages"
